...
import { dosavegrower, dosearchgrower } from '../services/user-controller';
...
export default function GrowerProfile() {
    ...
    function doupdatepic(event) {
        setObj({ ...obj, ["pic1"]: event.target.files[0] });
    }
    ...
    async function dosave() {
        ...
        const serverMsg = await dosavegrower(formData);
    ...
    async function doupdateProfile() {
        const url = `http://mern-project-2024.onrender.com/profile/update-grower-profile`;
        const formdata = new FormData();
        for (const prop in obj) {
            formdata.append(prop, obj[prop]);
        }
    ...
    async function dofetchprofile() {
        const serverMsg = await dosearchgrower(obj.email);
        ...
        <form className="container mx-auto px-4 py-8 bg-gray-100 rounded-lg shadow-md">
            <div className="border-b border-gray-200 pb-8 mb-8">
                ...
                <div className="flex items-center">
                    ...
                    <input type="file" name="pic1" id="pic1" className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" onChange={doupdatepic} />
                </div>
